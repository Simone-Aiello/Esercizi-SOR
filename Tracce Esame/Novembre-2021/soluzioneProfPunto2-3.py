...
class UnDueTreStella:
    ...
    def __init__(self, specialMode = False):
        ...
        self.lock = RLock()
        #
        # PUNTO 3: introduco una condizione per segnalare a tutti i thread, tra cui il Timer, che il gioco Ã¨ finito per qualche motivo.
        # 
        ...
        # bisognerÃ  poi aspettare che il Killer finisca tutte le uccisioni, prima di stampare le statistiche finali e terminare
        #
        self.killerFinished = False
        self.condFineKilling = Condition(self.lock)
        #
        # AGGIUNTA MIGLIORATIVA OPZIONALE: una Barriera per stampare le scritte finali solo quando StellaMaster, Display, Killer, Timer e tutti i Player hanno correttamente finito le loro attivitÃ .
        #
        ...
        with self.lock:
            ...
            if self.currentMode == GREEN and not (self.specialMode and self.playerSalvi >= 50):
                ...
                if self.playerData[num].position > self.winningLine:
                    ...
                    #
                    # PUNTO 3: conteggio i salvi ed eventualmente avvio il Timer. Inoltre blocco il Timer se il gioco finisce perchÃ¨ ci sono 50 salvi.
                    #
                    self.playerSalvi += 1
                ...
                #
                # Punto 2: la kill non va piÃ¹ fatta direttamente
                #
                # self.kill(num)
    ...
    # Punto 3:
    #
    # se siamo in specialMode, il gioco puÃ² terminare per un terzo motivo, e cioÃ¨ i giocatori salvi sono esattamente 50.
    #
    ...
    def run(self):
        ...
        while aliveAndKicking > 0:
            if self.game.getLight() == GREEN:
                #
                # Questa sleep riduce la prontezza di riflessi di un Player, aumentando la probabilitÃ  di trovare la luce RED quando viene effettuato uno step
                #
