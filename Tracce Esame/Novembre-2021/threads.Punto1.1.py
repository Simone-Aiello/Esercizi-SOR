...
#VERSIONE CON SOLO LA STAMPA DI KILL IN SYNC, LE URLA DEL PLAYER POSSONO ESSERE STAMPATE DOPO CHE IL SEMAFORO DIVENTA GREEN
...
class UnDueTreStella:
    ...
    def setLight(self, v : int):
        with self.lock:
            self.currentMode = v
            if self.currentMode == GREEN:
                self.shout("green light")
            else:
                self.shout("red light")
    #
