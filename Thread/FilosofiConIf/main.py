...
class Tavolo:
    ...
    def prendiMangiaESmetti(self,posizione):
        mia_bacchetta  = self.bacchetta[posizione]
        altra_bacchetta = self.bacchetta[(posizione + 1) % 5]
        try:
            if posizione%2 == 0:
                mia_bacchetta.lock.acquire()
                print(f"{posizione} prende mia")
                altra_bacchetta.lock.acquire()
                print(f"{posizione} prende altra")
            else:
                altra_bacchetta.lock.acquire()
                print(f"{posizione} prende altra")
                mia_bacchetta.lock.acquire()
            ...
            print(f"il filoso {posizione} mangia")
            sleep(1)
        finally:
            mia_bacchetta.lock.release()
            altra_bacchetta.lock.release()
